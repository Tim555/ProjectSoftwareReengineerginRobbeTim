/* LittleDarwin generated mutant
 ...
 ----> before: drState   . lowerCoordinates  . size  ( )  + drState   . upperCoordinates  . size  ( )  
----> after: drState   . lowerCoordinates  . size  ( )  - drState   . upperCoordinates  . size  ( )  
...
package org . jfree . chart . renderer . xy  ;
 ...
 public  class DeviationRenderer extends XYLineAndShapeRenderer   { public   static   class State extends XYLineAndShapeRenderer . State   { public   List   upperCoordinates    ;
  ...
  this   . upperCoordinates  = new java . util . ArrayList  ( )       ;
   ...
   public   DeviationRenderer ( )  { this   ( true    , true     )   ;
  ...
  }      public   void setAlpha ( float   alpha    )  { if ( alpha   <0.0f     || alpha   >1.0f      )  { throw new IllegalArgumentException  ( "Requires 'alpha' in the range 0.0 to 1.0."     )     ;
  }     this   . alpha  = alpha     ;
  fireChangeEvent   ( )   ;
  }      @ Override      public   void setDrawSeriesLineAsPath ( boolean   flag    )  { }      @ Override      public   Range   findRangeBounds ( XYDataset   dataset    )  { return findRangeBounds   ( dataset   , true     )  ;
  }      @ Override      public   XYItemRendererState   initialise ( Graphics2D   g2   , Rectangle2D   dataArea   , XYPlot   plot   , XYDataset   dataset   , PlotRenderingInfo   info    )  { State   state  = new State  ( info    )         ;
  state   . seriesPath  = new GeneralPath  ( )       ;
  state   . setProcessVisibleItemsOnly  ( false     )   ;
  return state   ;
  }      @ Override      public   int   getPassCount ( )  { return 3    ;
  }      @ Override      protected   boolean   isItemPass ( int   pass    )  { return ( pass   == 2     )   ;
  }      @ Override      protected   boolean   isLinePass ( int   pass    )  { return ( pass   == 1     )   ;
  ...
  }     if ( pass   == 0     )  { IntervalXYDataset   intervalDataset  = ( IntervalXYDataset   ) dataset        ;
  ...
  if ( orientation   == PlotOrientation   . HORIZONTAL   )  { drState   . lowerCoordinates  . add  ( new double   [ ] { yyLow    , xx    }      )   ;
  drState   . upperCoordinates  . add  ( new double   [ ] { yyHigh    , xx    }      )   ;
  }   else if ( orientation   == PlotOrientation   . VERTICAL   )  { drState   . lowerCoordinates  . add  ( new double   [ ] { xx    , yyLow    }      )   ;
  drState   . upperCoordinates  . add  ( new double   [ ] { xx    , yyHigh    }      )   ;
  }      if ( item   == ( dataset   . getItemCount  ( series    )  - 1     )    )  { Composite   originalComposite  = g2   . getComposite  ( )      ;
...
i   --     ) { coords   = ( double  [ ]  ) drState   . upperCoordinates  . get  ( i    )     ;
  ...
  }     }     if ( isLinePass   ( pass    )  )  { if ( item   == 0     )  { State   s  = ( State   ) state        ;
  s   . seriesPath  . reset  ( )   ;
  s   . setLastPointGood  ( false     )   ;
  }     if ( getItemLineVisible   ( series   , item    )  )  { drawPrimaryLineAsPath   ( state   , g2   , plot   , dataset   , pass   , series   , item   , domainAxis   , rangeAxis   , dataArea    )   ;
  ...
  }     if ( ! ( obj   instanceof DeviationRenderer    )    )  { return false    ;
